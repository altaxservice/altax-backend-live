...
/** Live-computed preview before filing — same numbers mark-filed will use, no DB write. */
form941FilingsRouter.get("/preview", requireAuth, requireRole("admin", "staff"), asyncHandler(async (req: AuthedRequest, res: Response) => {
  const clientId = String(req.query.clientId || "").trim();
  const loaded = await loadClient(req, clientId);
  if ("error" in loaded) return res.status(loaded.status).json({ error: loaded.error });
  ...
  const totals = await computeForm941Quarter(clientId, parsed.year, parsed.quarter);
  const eftpsDepositsApplied = await sumEftpsDepositsInPeriod(clientId, period.start, period.end);
  const balanceDue = totals.grossLiability - eftpsDepositsApplied;
  res.json({ ...totals, eftpsDepositsApplied, balanceDue, periodStart: period.start, periodEnd: period.end, dueDate: form941DueDate(parsed.year, parsed.quarter) });
}));
  ...
  const row = await queryOne<{ share_token: string }>(
    `INSERT INTO altax.v3_form941_filings
     ...
     ON CONFLICT (client_id, period_end) DO UPDATE SET
       employee_count = EXCLUDED.employee_count, wages = EXCLUDED.wages, federal_withholding = EXCLUDED.federal_withholding,
       social_security_wages = EXCLUDED.social_security_wages, medicare_wages = EXCLUDED.medicare_wages,
       gross_liability = EXCLUDED.gross_liability, eftps_deposits_applied = EXCLUDED.eftps_deposits_applied,
       balance_due = EXCLUDED.balance_due, filed_date = EXCLUDED.filed_date, paid_date = EXCLUDED.paid_date,
       filed_by = EXCLUDED.filed_by, filed_at = now(),
       share_token = COALESCE(altax.v3_form941_filings.share_token, EXCLUDED.share_token)
    ...
    [client.clientId, period.start, period.end, parsed.quarter, totals.employeeCount, totals.wages, totals.federalWithholding,
      ...
      filedDate, paidDate, req.user!.email, newShareToken]
  );

...
checklistsRouter.post("/clients/:clientId/checklist/:progressId/toggle", requireAuth, requireRole("admin", "staff"), asyncHandler(async (req: AuthedRequest, res: Response) => {
  ...
  await query(
    `UPDATE altax.v3_client_checklist_progress SET checked = $2, checked_at = $3, checked_by = $4 WHERE progress_id = $1`,
    [progressId, checked, checked ? new Date() : null, checked ? req.user!.email : null]
  );

  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div ref={panelRef} className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="recurring-billing-modal-title" style={{ maxWidth: 620 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="field"><label htmlFor="recur-description">Description</label><input id="recur-description" value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} placeholder="e.g. Monthly bookkeeping service" /></div>
        <div className="form-grid">
          <div className="field"><label htmlFor="recur-amount">Amount</label><input id="recur-amount" type="number" step="0.01" min="0.01" value={form.amount} onChange={(e) => setForm((f) => ({ ...f, amount: e.target.value }))} /></div>
          ...
          <div className="field"><label htmlFor="recur-auto-send">Auto Send Invoice</label><select id="recur-auto-send" value={form.autoSendInvoice ? "yes" : "no"} onChange={(e) => setForm((f) => ({ ...f, autoSendInvoice: e.target.value === "yes" }))}><option value="no">No</option><option value="yes">Yes</option></select></div>
          <div className="field"><label htmlFor="recur-status">Status</label><select id="recur-status" value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value }))}><option>Active</option><option>Paused</option><option>Archived</option></select></div>
        </div>
        <div className="field"><label htmlFor="recur-notes">Notes</label><textarea id="recur-notes" rows={2} value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} /></div>

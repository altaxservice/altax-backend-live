...
interface PublicAnnualReportFiling {
...
/**
 * Public, no-login MD Annual Report filing view + acknowledge — the destination
 * of the "View & Acknowledge Report" link in the Save & Send email. Same shape
 * as PublicMdFilingPage minus the PDF download (no generator exists for this type).
 */
export function PublicAnnualReportPage() {
  const { token } = useParams<{ token: string }>();
  const [filing, setFiling] = useState<PublicAnnualReportFiling | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [acknowledging, setAcknowledging] = useState(false);

  function load() {
    if (!token) return;
    api.get<{ filing: PublicAnnualReportFiling }>(`/public/annual-report/${token}`)
  ...
  async function handleAcknowledge() {
    ...
    try {
      await api.post(`/public/annual-report/${token}/acknowledge`, {});
      load();
    } catch (err) {
    ...
    <PublicPageShell>
      <div style={pageStyle}>
        <div style={{ borderLeft: "4px solid var(--teal)", paddingLeft: 16, marginBottom: 4 }}>
          ...
          <h1 style={{ fontSize: 24, margin: "4px 0 0", fontWeight: 800, letterSpacing: -0.3 }}>Maryland Annual Report Filing</h1>
          <p className="muted" style={{ fontSize: 13.5, margin: "6px 0 0" }}>Report Year {String(filing.period_start).slice(0, 4)}</p>
        </div>

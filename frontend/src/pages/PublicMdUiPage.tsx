...
/**
 * Public, no-login MD UI filing view + acknowledge — the destination of the
 * "View & Acknowledge Report" link in the Save & Send email. Same shape as
 * PublicAnnualReportPage.
 */
export function PublicMdUiPage() {
  const { token } = useParams<{ token: string }>();
  const [filing, setFiling] = useState<PublicMdUiFiling | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [acknowledging, setAcknowledging] = useState(false);

  function load() {
    if (!token) return;
    api.get<{ filing: PublicMdUiFiling }>(`/public/md-ui/${token}`)
  ...
  async function handleAcknowledge() {
    ...
    try {
      await api.post(`/public/md-ui/${token}/acknowledge`, {});
      load();
    } catch (err) {
